...
> **ACTIVE WORK: v2 design re-skin (Phases 0–4 inline-hex sweep ✅, Phase 4c next).** New design system (sage/cream, Inter/Inter Tight/JetBrains Mono, OS-seeded dark mode) on branch `redesign-v2`. **Visual re-skin only** — no screen/data/logic changes. **Resume point:** Phase 4c — delete the v1 `colors`/`shadow`/`fontSizes` block + `theme` aggregate from `theme.ts`, delete the back-compat font aliases (`serif`/`sans*`) from `theme.ts`+`tailwind.config.js` and migrate every `font-serif`/`font-sans*` call-site (all screens), repoint the `_layout.tsx` `colors.bg` v1 ref, then full light+dark Puppeteer+Safari QA of all 5 screens. ⚠️ Phase 4c is a broad-blast-radius multi-file refactor — confirm with user before starting (§3). Exact conventions + commit trail in §9b. `testingprod` remains the shippable v1; all v2 work is committed locally on `redesign-v2` (not pushed).
...
| 4c | ⏳ **NEXT — broad blast radius, confirm w/ user first (§3).** Now that `product/[id].tsx` no longer imports v1 `colors`: delete the v1 `colors`/`shadow`/`fontSizes` block + `theme` aggregate from `theme.ts`, delete the back-compat font aliases (`serif`/`sans*`) from `theme.ts`+`tailwind.config.js` and migrate **every** `font-serif`/`font-sans*` class call-site (all 5 screens + shared comps), repoint the `_layout.tsx` `colors.bg` v1 ref + fix the stale `className="bg-green"` doc-comment. Then full light+dark Puppeteer + Safari sweep of all 5 screens. typecheck = 0 gate. | `theme.ts`, `tailwind.config.js`, `_layout.tsx`, every comp w/ `font-*` class | H |
